...
def run_register(driver):  
    ...
    while bcl_2:
        ...
        driver.get(url)
        time.sleep(5)
        temp_tab = driver.current_window_handle
        email_acc = driver.execute_script("return document.querySelector('[id=\"email\"]').innerText")
        print('email_acc: ', email_acc)
        time.sleep(2)
        driver.switch_to.new_window('tab')
        url ="https://cp.sprinthost.ru/customer/sites/index"
        driver.get(url)
        sprint_tab = driver.current_window_handle
        thus = True
                ...
                if code_ == None:
                    ...
                    while th_tem:
                        temp_src ="""
                            // Get the sent Code
                            let code = "no"
                            let list = document.querySelectorAll('td[class="from"]'); 
                            for (let i = 0; i < list.length; i++) {
                                if(list[i].nextElementSibling.innerText.includes('Код для подтверждения')){
                                    const numbers = list[i].nextElementSibling.innerText.match(/\d+/g).join("");
                                    code = numbers;
                                }
                            }
                            return code;
                        ...
                        code_ = driver.execute_script(temp_src)
                        print('code_: ', code_)
                        if code_ != "no":
                            th_tem = False
                driver.switch_to.window(sprint_tab)
                th_check = True
                while th_check:
                    for lt in code_:
                        scs = f"""
