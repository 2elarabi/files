...
def solve_captcha(driver_, index_):
    ...
    while thuss:
        try:
            anch = driver_.find_element(By.ID, "recaptcha-anchor")
            thuss = False
        except:
            pass
    ...
    while thus_2:
            ...
            if anch.get_attribute("aria-checked") == "true":
                ...
                while thus_1:
                    if inc < 5:
                        try:
                            #btnns = driver_.execute_script("return document.querySelectorAll('.cl-v3-btn-base.cl-v3-btn-base--main.cl-v3-btn-base--lg')")
                ...
                thus_2 = False
                driver_.switch_to.default_content()
                captchas = driver_.execute_script("return document.querySelectorAll('[title=\"recaptcha challenge expires in two minutes\"]')")
                print('captchas: ', len(captchas))
                driver_.switch_to.frame(captchas[0])
                bttns = driver_.find_elements(By.TAG_NAME, "button")
                ...
                for bttn in bttns:
                    try:
                        if bttn.get_attribute("class") == "rc-button goog-inline-block rc-button-audio":
                            ...
                            driver_.execute_script("document.getElementsByClassName('rc-button goog-inline-block rc-button-audio')[0].click()")
                    except:
                        pass
                ...
                for bttn in bttns:
                    try:
                        if bttn.get_attribute("class") == "rc-button-default goog-inline-block":
                            ...
                            driver_.execute_script("document.getElementsByClassName('rc-button-default goog-inline-block')[0].click()")
                    except:
                        pass
                time.sleep(3)
                auds = driver_.find_elements(By.TAG_NAME, "a")
                bttns = driver_.find_elements(By.TAG_NAME, "button")
                print('auds: ', len(auds))
                for aud in auds:
                    if aud.get_attribute("class") == "rc-audiochallenge-tdownload-link":
                        ...
                        bttns = driver_.find_elements(By.TAG_NAME, "input")
                        for bttn in bttns:
                            try:
                                if bttn.get_attribute("id") == "audio-response":
                                    bttn.send_keys(result["text"], Keys.ENTER)
                                    time.sleep(2)
                            except:
                                pass
                        break
                ...
                trthu = True
                while trthu:
                    btntns = driver_.find_elements(By.TAG_NAME, "div")
                    for btntn in btntns:
                        try:
                            if btntn.get_attribute("class") == "rc-audiochallenge-error-message":
                                text = driver_.execute_script("return arguments[0].innerText;", btntn)
                                if "Multiple correct solutions" in text:
                                    btntnss = driver_.execute_script('return document.getElementsByClassName("rc-button goog-inline-block rc-button-reload")')
                                    for btntnss_ in btntnss:
                                        try:
                                            if btntnss_.get_attribute("class") == "rc-button goog-inline-block rc-button-reload":
                                                ...
                                                driver_.execute_script('return document.getElementsByClassName("rc-button goog-inline-block rc-button-reload")[0].click()')
                                                time.sleep(2)
                                                bttns = driver_.find_elements(By.TAG_NAME, "button")
                                                for bttn in bttns:
                                                    try:
